...
nltk.download('omw-1.4')  # Optional: downloads additional data for WordNet
...
def preprocess():
    try:
        ...
        # Load the dataset and print column names to verify
        dataset = pd.read_csv(filename, encoding='utf-8', nrows=300)
        text.insert(END, "Dataset loaded successfully.\n")
        text.insert(END, "Columns: " + str(dataset.columns) + "\n")
        
        # Iterate over the dataset
        for i in range(len(dataset)):
            msg = dataset.at[i, 'comment_text'] if 'comment_text' in dataset else None
            toxic = dataset.at[i, 'toxic'] if 'toxic' in dataset else None
            severe_toxic = dataset.at[i, 'severe_toxic'] if 'severe_toxic' in dataset else None
            obscene = dataset.at[i, 'obscene'] if 'obscene' in dataset else None
            threat = dataset.at[i, 'threat'] if 'threat' in dataset else None
            insult = dataset.at[i, 'insult'] if 'insult' in dataset else None
            identity_hate = dataset.at[i, 'identity_hate'] if 'identity_hate' in dataset else None
            
            # Only process if comment and labels are valid
            if msg is not None and all(x is not None for x in [toxic, severe_toxic, obscene, threat, insult, identity_hate]):
                ...
                text.insert(END, f"Skipped row {i} due to missing values.\n")
                
        Y1, Y2, Y3, Y4, Y5, Y6 = map(np.asarray, [Y1, Y2, Y3, Y4, Y5, Y6])
        text.insert(END, "Preprocessing completed successfully.\n")
    except Exception as e:
        text.insert(END, f"Error in preprocessing: {e}\n")
